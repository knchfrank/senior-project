...
def recognition():
    ...
    users = face_recognition.load_image_file(os.path.join(path, "pi/catkin_ws/src/testnav/src", "temp2.jpg"))
    
    face_locations = face_recognition.face_locations(users, number_of_times_to_upsample=0)
    
    known_face_encodings = []
    for i in range(len(face_locations)):
        user = face_recognition.face_encodings(users)[i]
        known_face_encodings.append(user)
